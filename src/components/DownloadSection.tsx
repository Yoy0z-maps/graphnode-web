...
const featureKeys = [
  { key: "markdownNote", icon: "Notebook", accent: "from-cyan-400/30 to-sky-500/10" },
  { key: "aiChat", icon: "AI Layer", accent: "from-fuchsia-400/30 to-violet-500/10" },
  { key: "graphVisualization", icon: "Graph View", accent: "from-blue-400/30 to-indigo-500/10" },
  { key: "smartSearch", icon: "Smart Find", accent: "from-emerald-400/30 to-teal-500/10" },
  { key: "agent", icon: "Agent Flow", accent: "from-amber-300/30 to-orange-500/10" },
...
const orbitingIcons = [
  { id: "brand", kind: "logo" as const, symbol: "", className: "left-4 top-8 sm:left-4 sm:top-10" },
  { id: "spark", kind: "symbol" as const, symbol: "✦", className: "right-2 top-12 sm:right-0 sm:top-16" },
  { id: "link", kind: "symbol" as const, symbol: "⌘", className: "left-0 bottom-24 sm:left-0 sm:bottom-28" },
  { id: "agent", kind: "symbol" as const, symbol: "◈", className: "right-6 bottom-18 sm:right-8 sm:bottom-20" },
  { id: "graph", kind: "symbol" as const, symbol: "◎", className: "left-18 -top-2 sm:left-20 sm:-top-3" },
  { id: "node", kind: "symbol" as const, symbol: "✳", className: "right-14 -bottom-2 sm:right-16 sm:-bottom-4" },
] as const;
...
export default function DownloadSection() {
  ...
  > = {
    ...
    microscope: {
      ...
      title: t("download.productProof.spotlights.microscope.title", {
        defaultValue: "Workspace-based deep analysis for notes and conversations",
      }),
      description: t("download.productProof.spotlights.microscope.description", {
        defaultValue:
    ...
    agent: {
      ...
      points: [
        ...
        t("download.productProof.spotlights.agent.points.2", {
          defaultValue: "Quick actions for summary, notes, relation, and importance",
        }),
      ],
    },
    "search-sync": {
      ...
      title: t("download.productProof.spotlights.searchSync.title", {
        defaultValue: "Fast local search backed by sync logic that respects recency",
      }),
      description: t("download.productProof.spotlights.searchSync.description", {
        defaultValue:
  ...
  const platforms = [
    {
      ...
      description: t("download.platforms.macosDescription", {
        defaultValue: "Optimized for Apple Silicon and Intel with a native desktop feel.",
      }),
    },
    {
      name: "Windows",
      ...
      description: t("download.platforms.windowsDescription", {
        defaultValue: "Fast install, polished interface, and smooth graph interaction on Windows.",
      }),
    },
    {
      name: "Linux",
      ...
      description: t("download.platforms.linuxDescription", {
        defaultValue: "Linux support is on the roadmap and will arrive with a dedicated release.",
      }),
    },
    {
  ...
  const previewNotes = [
    {
      ...
      excerpt: "Hero redesign, animated device preview, and tighter product story.",
    },
    {
      title: "Microscope UX Ideas",
      excerpt: "Workspace grouping, node context handoff, and graph zoom feedback.",
    },
    {
      title: "Search + Sync Summary",
      excerpt: "LWW sync, local-first notes, and command-surface style retrieval.",
    },
    {
      title: "Writing is telepathy",
      excerpt: "Ideas can travel through time and space without being uttered out loud.",
    },
  ];
  const previewNodes = [
  ...
  const noteTree = [
    ...
    { id: "writing-is-telepathy", name: "Writing is telepathy", type: "note" as const, depth: 1, parentId: "ideas" },
    { id: "calmness-is-a-superpower", name: "Calmness is a superpower", type: "note" as const, depth: 1, parentId: "ideas" },
    ...
    { id: "specialization-is-for-insects", name: "Specialization is for insects", type: "note" as const, depth: 1, parentId: "references" },
    { id: "first-principles", name: "First principles", type: "note" as const, depth: 1, parentId: "references" },
    { id: "books", name: "Books", type: "folder" as const, depth: 0 },
    { id: "on-writing", name: "On Writing", type: "note" as const, depth: 1, parentId: "books" },
    { id: "philosophy", name: "Philosophy", type: "folder" as const, depth: 0 },
    { id: "what-if-it-were-easy", name: "What if it were easy", type: "note" as const, depth: 1, parentId: "philosophy" },
    { id: "tools", name: "Tools", type: "folder" as const, depth: 0 },
    { id: "creativity-is-combinatorial", name: "Creativity is combinatorial", type: "note" as const, depth: 1, parentId: "tools" },
    { id: "research", name: "Research", type: "folder" as const, depth: 0 },
    { id: "graph-ranking-notes", name: "Graph ranking notes", type: "note" as const, depth: 1, parentId: "research" },
    { id: "semantic-retrieval", name: "Semantic retrieval", type: "note" as const, depth: 1, parentId: "research" },
    { id: "product", name: "Product", type: "folder" as const, depth: 0 },
    { id: "launch-checklist", name: "Launch checklist", type: "note" as const, depth: 1, parentId: "product" },
    { id: "beta-feedback", name: "Beta feedback", type: "note" as const, depth: 1, parentId: "product" },
  ];
  ...
  > = {
    ...
    "graph-ranking-notes": {
      ...
      tag: "#research",
      source: "Research",
      content: ["## Ranking signals", "Connected notes should rank higher when semantic similarity and explicit links agree."],
    },
    "semantic-retrieval": {
      title: "Semantic retrieval",
      tag: "#retrieval",
      source: "Research",
      content: ["## Retrieval behavior", "Search should feel fast, but the result order should still expose why a note appeared."],
    },
    "launch-checklist": {
      title: "Launch checklist",
      tag: "#product",
      source: "Product",
      content: ["## Before launch", "- Finalize copy", "- Review onboarding", "- Validate pricing", "- Check downloads"],
    },
    "beta-feedback": {
      title: "Beta feedback",
      tag: "#product",
      source: "Product",
      content: ["## Repeating themes", "People like the concept, but want stronger first-run clarity and more obvious note-to-graph relationships."],
    },
  };
  ...
  const selectedNoteDocument = selectedNoteId ? noteDocuments[selectedNoteId] ?? null : null;
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute left-0 top-[62%] z-0 -translate-x-1/3 opacity-[0.05]">
        <img src={logo} alt="" className="h-[280px] w-[280px]" />
      </div>
      ...
      <div className="pointer-events-none absolute right-0 top-1/3 z-0 translate-x-1/3 opacity-[0.07]">
        <img src={logo} alt="" className="h-[340px] w-[340px]" />
      </div>
            ...
            >
              ...
              >
                {t("download.hero.eyebrow", {
                  defaultValue: "A more alive way to think, write, and connect ideas",
                })}
              </motion.p>
            ...
            >
              ...
              {orbitingIcons.map((item, index) => {
                ...
                return (
                  <motion.div
                    key={item.id}
                    initial={{ x: scatter.dx, y: scatter.dy, rotate: scatter.initR, opacity: 0 }}
                    animate={isPreviewInView
                      ? { x: 0, y: 0, rotate: 0, opacity: 1 }
                      : { x: scatter.dx, y: scatter.dy, rotate: scatter.initR, opacity: 0 }
                    }
                    transition={{ type: "spring", stiffness: 55, damping: 14, delay: scatter.delay, opacity: { duration: 0.3, delay: scatter.delay } }}
                    className={`absolute z-20 hidden md:block ${item.className}`}
                  >
                    <motion.div
                      animate={isPreviewInView ? {
                        y: [0, scatter.floatY, 0],
                        rotate: [0, scatter.floatR, scatter.floatR * -0.4, 0],
                      } : {}}
                      transition={{
                    ...
                    >
                      {item.kind === "logo" ? (
                        ...
                        <span className="text-lg font-bold text-cyan-100">{item.symbol}</span>
                      )}
                    </motion.div>
                  </motion.div>
                );
              ...
              <div className="relative w-full max-w-[1180px] overflow-x-hidden rounded-[28px] border border-white/12 bg-slate-950/60 p-3 shadow-[0_30px_120px_rgba(2,6,23,0.55)] backdrop-blur-xl sm:rounded-[32px] sm:p-4">
                <div className="rounded-[24px] border border-white/8 bg-[radial-gradient(circle_at_top,#172554_0%,#0f172a_55%,#020617_100%)] p-4 sm:rounded-[26px] sm:p-5">
                  <div
                    className="overflow-hidden rounded-[24px] border border-[#303036] bg-[#191919]"
                    style={{
                      fontFamily:
                        '"Noto Sans KR","Apple SD Gothic Neo","Malgun Gothic",sans-serif',
                    }}
                  >
                    <div className="flex items-center justify-between border-b border-[#35353a] bg-[#232324] px-4 py-[9px]">
                      <div className="flex items-center gap-[7px]">
                        <span className="h-[11px] w-[11px] rounded-full bg-[#ff5f57]" />
                        <span className="h-[11px] w-[11px] rounded-full bg-[#febc2e]" />
                        <span className="h-[11px] w-[11px] rounded-full bg-[#28c840]" />
                      </div>
                      <p className="text-[12px] font-medium text-[#8b8b91]">
                        {previewTab === "home"
                          ? "graphnode.app / home"
                          : previewTab === "chat"
                            ? "graphnode.app / chat"
                    ...
                    <div
                      className="relative grid h-[620px] w-full overflow-hidden grid-cols-[45px_minmax(0,1fr)]"
                    >
                      ...
                      <div
                        className="relative z-10 flex flex-col justify-between border-r border-[#2d2d32] bg-[#1f1f20] px-[8px] py-[10px]"
                      >
                        <div className="flex flex-col items-center gap-2">
                          {/* Home icon */}
                          <motion.div
                            initial={{ x: sidebarIconScatter[0].x, y: sidebarIconScatter[0].y, rotate: sidebarIconScatter[0].rotate, scale: sidebarIconScatter[0].scale, opacity: 0.9 }}
                            animate={isPreviewInView
                              ? { x: 0, y: 0, rotate: 0, scale: 1, opacity: 1 }
                              : { x: sidebarIconScatter[0].x, y: sidebarIconScatter[0].y, rotate: sidebarIconScatter[0].rotate, scale: sidebarIconScatter[0].scale, opacity: 0.9 }
                            }
                            transition={{ type: "spring", stiffness: 70, damping: 14, delay: sidebarIconScatter[0].delay }}
                          >
                            <button
                              ...
                              aria-label="Home"
                            >
                              <img src={logo} alt="GraphNode" className="h-4 w-4" />
                            </button>
                          </motion.div>
                          ...
                          {previewNavItems.map((item, index) => {
                            ...
                            return (
                              <motion.div
                                key={item.id}
                                initial={{ x: scatter.x, y: scatter.y, rotate: scatter.rotate, scale: scatter.scale, opacity: 0.9 }}
                                animate={isPreviewInView
                                  ? { x: 0, y: 0, rotate: 0, scale: 1, opacity: 1 }
                                  : { x: scatter.x, y: scatter.y, rotate: scatter.rotate, scale: scatter.scale, opacity: 0.9 }
                                }
                                transition={{ type: "spring", stiffness: 70, damping: 14, delay: scatter.delay }}
                                  >
                                <button
                                  onClick={() => setPreviewTab(item.id)}
                                  className={`flex h-[28px] w-[28px] items-center justify-center rounded-[6px] text-sm font-semibold transition ${
                                    active
                                      ? "bg-[#7ab5fb] text-white"
                                      : "bg-transparent text-[#6b7280] hover:bg-[#1e3a5f] hover:text-white"
                                  }`}
                                  aria-label={item.label}
                                >
                                  <PreviewNavIcon id={item.id} active={active} />
                                </button>
                              </motion.div>
                            );
                          })}
                        ...
                        <div className="flex flex-col items-center gap-2">
                          <div className="flex h-[28px] w-[28px] items-center justify-center rounded-full bg-[#d4d4d8] text-[11px] font-semibold text-black">
                            A
                          </div>
                        </div>
                      </div>
                      ...
                      >
                        <div
                          className={`border-r border-[#2d2d32] bg-[#262627] pl-3 pr-1 py-4 ${
                            previewTab === "home" ? "invisible overflow-hidden" : "flex min-h-0 h-[620px] flex-col overflow-hidden"
                          }`}
                        >
                          <div className="app-preview-scrollbar h-full min-h-0 flex-1 overflow-y-auto pr-0">
                            ...
                            {previewTab === "chat" &&
                              <>
                                <button className="mb-2 flex h-[32px] w-full items-center gap-[6px] rounded-[6px] px-[6px] text-left text-[14px] text-[#d1d1d6] transition hover:bg-[#303034] hover:text-[#7ab5fb]">
                                  <PlusIcon />
                                  <span>New Chat</span>
                                </button>
                                {chatRooms.map((thread) => (
                                  <button
                                    ...
                                    className={`block h-[32px] w-full rounded-[6px] border px-[6px] py-[5.5px] text-left transition ${
                            ...
                            {previewTab === "notes" && (
                              <>
                                ...
                                <p className="mb-[6px] mt-2 text-[12px] font-normal text-[#a1a1aa]">Workspace</p>
                                {visibleNoteTree.map((item) => (
                                  <button
                                      ...
                                      if (item.type === "folder") {
                                        setExpandedFolderIds((current) =>
                                          current.includes(item.id)
                                            ? current.filter((folderId) => folderId !== item.id)
                                            : [...current, item.id],
                                        );
                                      } else {
                                    ...
                                    className={`flex h-[32px] w-full items-center gap-1 rounded-[6px] px-[6px] text-left text-[14px] transition ${
                                      item.type === "note" && selectedNoteId === item.id
                                        ? "bg-[#474748] text-white"
                                        : "text-[#d1d1d6] hover:bg-[#303034]"
                                    }`}
                                    style={{ paddingLeft: item.depth === 1 ? 22 : 6 }}
                                  >
                                    <span className={`w-3 text-[11px] ${item.type === "note" ? "text-transparent" : "text-[#8f8f95]"}`}>
                                      {item.type === "folder"
                            ...
                            {previewTab === "visualize" &&
                              graphCollections.map(
                                (item, index) => (
                                  ...
                                  >
                                    <p className="truncate text-[14px]">{item}</p>
                                  </button>
                                ),
                              )}
                            </div>
                        ...
                        <div className="min-w-0 overflow-hidden bg-[#1d1d1f] p-0">
                          {previewTab === "home" && (
                            <div className="relative h-[620px] overflow-hidden bg-[#1f1f1f]">
                              ...
                              <motion.div
                                ...
                                initial={{ opacity: 1 }}
                                animate={isPreviewInView ? { opacity: 0 } : { opacity: 1 }}
                              ...
                              >
                                <div className="flex w-full flex-col items-center pt-12">
                                  <div className="flex flex-col items-center gap-2 text-white">
                                    <p className="text-[24px] font-semibold">{t("preview.home.title")}</p>
                                    <p className="text-[18px] text-[#9090a5]">{t("preview.home.subtitle")}</p>
                                  </div>

                                  <div className="mx-auto mt-8 w-[560px] rounded-[14px] border border-white/10 bg-[#111317] px-4 py-4 shadow-[0_2px_20px_0_rgba(186,218,255,0.10)]">
                                    <div className="min-h-[36px] text-[13px] text-[#6f7682]">
                                      {t("preview.home.placeholder")}
                                    </div>
                                    <div className="mt-3 flex items-center justify-between">
                                      <div className="rounded-md border border-white/8 bg-[#1f2329] px-3 py-1.5 text-[12px] text-slate-300">
                                        GPT-5.4
                                      </div>
                                  ...
                                  <div className="mx-auto mt-10 w-full max-w-[680px] px-4 pb-8">
                                    <p className="mb-5 text-center text-[20px] font-medium text-white">{t("preview.home.recentNotes")}</p>
                                    <div className="grid grid-cols-3 gap-3">
                                      <div className="flex h-[160px] items-center justify-center rounded-[12px] border border-dashed border-white/12 bg-[#242427] text-[28px] text-white/30">
                                        +
                                      </div>
                                      {previewNotes.slice(0, 2).map((note) => (
                                        <div
                                          key={note.title}
                                          className="h-[160px] rounded-[12px] border border-white/8 bg-[#26272b] px-4 py-4"
                                        >
                                          ...
                                          <p className="mt-auto pt-6 text-[10px] text-[#7f7f85]">Apr 09, 2026</p>
                                        </div>
                                      ))}
                                    </div>
                                  </div>
                              ...
                              <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-5 py-5">
                                <div className="app-preview-scrollbar min-h-0 flex-1 overflow-y-scroll pr-0.5">
                                  <div className="space-y-6 pt-1">
                                    {selectedChatId && previewChatConversation.map((message, index) => (
                                      <div
                                        key={`preview-chat-${selectedChatId}-${index}`}
                                        className={`flex ${message.role === "user" ? "justify-end" : "justify-start"} items-start`}
                                      >
                                        {message.role === "user" ? (
                                          <div className="ml-20 max-w-full rounded-2xl rounded-tr-sm bg-[#2a2f39] px-4 py-3 text-[14px] leading-6 text-[#eef2ff]">
                                            {message.text}
                                          </div>
                                        ) : (
                                          <div className="w-full">
                                            <div className="flex items-start gap-3 rounded-2xl border border-white/8 bg-transparent p-5 text-[#d0d0d5] shadow-[0_2px_4px_0_rgba(25,33,61,0.08)]">
                                              <img src={logo} alt="GraphNode" className="h-6 w-6 shrink-0 pt-1" />
                                              <div className="min-w-0 whitespace-pre-line text-[14px] leading-7">{message.text}</div>
                                            </div>
                                            <div className="ml-1.5 mt-2 flex items-center gap-2.5 text-[#8c8c91]">
                                              <span className="rounded p-1 hover:bg-[#2a2a2b]">⧉</span>
                                              <span className="rounded p-1 hover:bg-[#2a2a2b]">↻</span>
                                            </div>
                                          </div>
                                        )}
                                ...
                                <div className="mt-5 rounded-xl border border-[rgba(255,255,255,0.1)] bg-[#111317] px-3 py-3 shadow-[0_2px_20px_0_rgba(186,218,255,0.16)]">
                                  <div className="min-h-[36px] text-[14px] text-slate-500">
                            ...
                            <div className="h-full overflow-hidden bg-[#1f1f1f]">
                              <div className="app-preview-scrollbar h-full overflow-y-scroll px-6 py-5">
                                {selectedNoteId && selectedNoteDocument ? (
                                  <div className="preview-markdown text-[14px] leading-7 text-[#d0d0d5]">
                                    <h2 className="mb-4 text-[30px] font-bold leading-tight text-white">
                                  ...
                                  <div className="flex h-full items-center justify-center text-[14px] text-[#6f7682]">
                              ...
                              <div className="mb-4 flex w-full items-center justify-between">
                                <div>
                                  <p className="text-xs text-slate-400">
                                    {t("download.hero.graphViewLabel", { defaultValue: "Graph View" })}
                                  </p>
                                  ...
                                  <div className="space-y-3">
                                    {[
                                      ...
                                      >
                                        ...
                                        <p className="mt-1 text-xs text-slate-400">12 linked nodes</p>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                                <div className="relative h-full min-h-[290px] overflow-hidden rounded-[22px] border border-[#2a2a2e] bg-[#202020]">
                                  ...
                                  >
                                    <path d="M18 26 C30 18, 40 17, 49 21" stroke="rgba(255,255,255,0.28)" strokeWidth="0.55" fill="none" />
                                    <path d="M49 21 C62 24, 71 25, 78 31" stroke="rgba(255,255,255,0.24)" strokeWidth="0.55" fill="none" />
                                    <path d="M22 67 C34 58, 44 45, 49 21" stroke="rgba(255,255,255,0.2)" strokeWidth="0.55" fill="none" />
                                    <path d="M22 67 C39 71, 51 72, 63 75" stroke="rgba(255,255,255,0.18)" strokeWidth="0.55" fill="none" />
                                  </svg>
                                </div>
                              </div>
                            </div>
          ...
          >
            <div>
              <p className="mb-4 text-sm font-semibold uppercase tracking-[0.28em] text-cyan-200/75">
                {t("download.features.eyebrow", { defaultValue: "Why it feels different" })}
              </p>
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {featureKeys.map((feature, index) => (
              <motion.div
                ...
                className="group relative overflow-hidden rounded-[28px] border border-white/10 bg-white/6 p-7 backdrop-blur-sm"
              >
                <div
      ...
      <section className="px-4 py-18 md:px-8 md:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.92fr_1.08fr] lg:items-stretch">
          <motion.div
            ...
            transition={{ duration: 0.7 }}
            className="flex h-full flex-col rounded-[32px] border border-white/10 bg-linear-to-br from-slate-900/90 via-slate-950/88 to-cyan-950/55 p-7 sm:p-9"
          >
            <p className="mb-5 text-sm font-semibold uppercase tracking-[0.28em] text-cyan-200/75">
              {t("download.workflow.eyebrow", { defaultValue: "From note to action" })}
            </p>
            <h2 className="text-4xl font-black tracking-[-0.04em] text-white md:text-5xl">
              {t("download.workflow.title", {
                defaultValue: "A workflow that looks dynamic because the product itself is dynamic",
              })}
            </h2>
            <p className="mt-7 max-w-xl text-base leading-7 text-slate-300">
              {t("download.workflow.description", {
                defaultValue:
                  "Each step is designed to make ideas feel connected, visible, and ready to act on inside one continuous GraphNode workflow.",
              })}
            </p>
          </motion.div>
          ...
          <div className="flex h-full flex-col gap-5">
            {workflowCards.map((card, index) => (
              <motion.div
                ...
                className="flex-1 rounded-[28px] border border-white/10 bg-white/6 p-6 backdrop-blur"
              >
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-4 py-2 text-sm font-medium text-cyan-100">
                    {t("download.workflow.signal", { defaultValue: "Interactive surface" })}
                  </div>
                </div>
                <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300">{card.text}</p>
              </motion.div>
            ))}
          </div>
        </div>
            ...
            <div>
              ...
              <h2 className="max-w-3xl text-4xl font-black tracking-[-0.04em] text-white md:text-6xl">
                {t("download.productProof.title", {
                  defaultValue: "Real product capabilities, turned into an interactive story",
                })}
              </h2>
            </div>
          </motion.div>

          <div className="grid gap-6 lg:min-h-[608px] lg:grid-cols-[0.42fr_0.58fr] lg:items-stretch">
            <div className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-2 lg:mx-0 lg:grid lg:h-[608px] lg:grid-rows-4 lg:gap-4 lg:overflow-visible lg:px-0 lg:pb-0">
              {productSpotlights.map((item, index) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`min-w-[280px] rounded-[26px] border p-5 text-left transition duration-300 lg:h-full lg:min-h-0 lg:min-w-0 ${
                      isActive
            ...
            <motion.div
              ...
              className="relative overflow-hidden rounded-[32px] border border-white/10 bg-slate-950/60 p-6 backdrop-blur-xl lg:h-[608px] md:p-8"
            >
              <div
              ...
              <div className="relative flex h-full flex-col">
                ...
                <div className="mt-6 grid gap-3 md:grid-cols-3">
                  {activeSpotlightContent.points.map((point, index) => (
                    <div
                      key={point}
                      className="min-h-[104px] rounded-[24px] border border-white/10 bg-white/6 p-4"
                    >
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
                        0{index + 1}
                      </p>
                      <p className="mt-2 text-sm leading-6 text-white">{point}</p>
                    </div>
                  ))}
                </div>

                <div className="app-preview-scrollbar mt-6 grow overflow-y-auto rounded-[28px] border border-white/10 bg-slate-900/70 p-5">
                  {activeSpotlight.id === "visualize" && (
                    <div className="grid gap-4 lg:grid-cols-[1.08fr_0.92fr]">
                      <div className="relative overflow-hidden rounded-[24px] border border-cyan-300/15 bg-[#081520] p-5">
                        ...
                        <div className="relative h-[220px]">
                          {[
                            { left: "10%", top: "24%", size: "h-10 w-10", label: "Notes" },
                            { left: "35%", top: "16%", size: "h-16 w-16", label: "Focus" },
                            { left: "68%", top: "20%", size: "h-12 w-12", label: "Cluster" },
                            { left: "22%", top: "62%", size: "h-12 w-12", label: "Agent" },
                            { left: "53%", top: "54%", size: "h-20 w-20", label: "Graph" },
                            { left: "80%", top: "63%", size: "h-10 w-10", label: "Ideas" },
                          ].map((node, index) => (
                            <div key={node.label}>
                              <div
                      ...
                      <div className="space-y-4">
                        <div className="rounded-[22px] border border-white/10 bg-white/6 p-4">
                          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-100/70">Summary</p>
                          <p className="mt-3 text-sm leading-6 text-white/90">
                            Move through the graph with nearby relations and focus summaries still in view.
                          </p>
                        </div>
                        <div className="rounded-[22px] border border-white/10 bg-white/6 p-4">
                          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-100/70">Path</p>
                          <div className="mt-3 flex flex-wrap gap-2">
                            {["Snapshot", "Focus node", "Relations", "Subcluster"].map((item) => (
                              <span key={item} className="rounded-full border border-white/10 bg-black/20 px-3 py-1.5 text-xs text-white/80">
                                {item}
                              </span>
                            ))}
                          </div>
                  ...
                  {activeSpotlight.id === "microscope" && (
                    <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
                      <div className="space-y-3">
                        {["Selected notes", "Conversation slice", "Workspace group"].map((item, index) => (
                          <div key={item} className="rounded-[20px] border border-white/10 bg-white/6 p-4">
                            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-fuchsia-100/70">0{index + 1}</p>
                            <p className="mt-2 text-sm text-white">{item}</p>
                          </div>
                        ))}
                      </div>
                      <div className="rounded-[24px] border border-fuchsia-300/15 bg-[#140c1b] p-5">
                        <div className="flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.18em] text-fuchsia-100/75">
                          <span className="rounded-full border border-fuchsia-300/20 bg-fuchsia-300/10 px-3 py-1.5">Microscope run</span>
                          <span>Grouped output</span>
                        </div>
                        <div className="mt-5 space-y-3">
                          {[
                            ["Patterns", "Recurring ideas are grouped into one analysis workspace."],
                            ["Signals", "High-similarity nodes surface as follow-up paths."],
                            ["Next action", "Selected findings can move directly into agent tools."],
                          ].map(([title, body]) => (
                  ...
                  {activeSpotlight.id === "agent" && (
                    <div className="grid gap-4 lg:grid-cols-[0.88fr_1.12fr]">
                      <div className="rounded-[22px] border border-amber-300/15 bg-[#171209] p-4">
                        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-100/70">Context attached</p>
                        <div className="mt-3 flex flex-wrap gap-2">
                          {["Linked note", "Graph cluster", "Microscope finding", "2 sources"].map((chip) => (
                            <span key={chip} className="rounded-full border border-amber-300/15 bg-amber-300/10 px-3 py-1.5 text-xs text-amber-50">
                              {chip}
                            </span>
                          ))}
                        </div>
                        <div className="mt-4 grid gap-2">
                          {["Summarize", "Extract actions", "Create note", "Map relations"].map((action) => (
                            <div key={action} className="rounded-[14px] border border-white/10 bg-white/6 px-3 py-2 text-sm text-white/85">
                              {action}
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="space-y-3">
                        <div className="ml-auto max-w-[85%] rounded-[20px] rounded-br-md border border-white/10 bg-white/8 px-4 py-3 text-sm leading-6 text-white">
                          Turn the selected materials into a brief and call out missing links.
                        </div>
                        <div className="max-w-[92%] rounded-[20px] rounded-bl-md border border-amber-300/15 bg-amber-300/10 px-4 py-3 text-sm leading-6 text-amber-50">
                          I grouped the context into three themes and drafted a note you can keep editing.
                        </div>
                        <div className="grid gap-2 md:grid-cols-2">
                          {["Source-aware answer", "Action result"].map((item) => (
                            <div key={item} className="rounded-[16px] border border-white/10 bg-white/6 px-4 py-3 text-sm text-slate-200">
                              {item}
                            </div>
                          ))}
                        </div>
                  ...
                  {activeSpotlight.id === "search-sync" && (
                    <div className="grid h-full gap-4 lg:grid-cols-[1fr_0.92fr]">
                      <div className="rounded-[24px] border border-emerald-300/15 bg-[#081712] p-4">
                        <div className="rounded-[16px] border border-white/10 bg-black/20 px-4 py-3 text-sm text-slate-300">
                        ...
                        <div className="mt-4 space-y-2">
                          {[
                            ...
                            <div key={title} className="flex items-center justify-between rounded-[16px] border border-white/10 bg-white/6 px-4 py-3">
                              <div>
                                <p className="text-sm text-white">{title}</p>
                                <p className="mt-1 text-xs text-slate-400">{meta}</p>
                              </div>
                              <span className="rounded-full bg-emerald-300/10 px-2.5 py-1 text-[11px] text-emerald-100">Hit</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      <div className="rounded-[24px] border border-white/10 bg-white/6 p-4">
                        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-100/70">Sync state</p>
                        <div className="mt-4 space-y-3">
                          {[
                            ...
                            <div key={title} className="flex items-start gap-3">
                              <span className={`mt-1 h-2.5 w-2.5 rounded-full ${state === "live" ? "bg-emerald-300 shadow-[0_0_14px_rgba(52,211,153,0.65)]" : "bg-slate-500"}`} />
                              <div className="rounded-[16px] border border-white/10 bg-black/15 px-4 py-3">
                                <p className="text-sm text-white">{title}</p>
            ...
            <div>
              ...
              <h2 className="max-w-3xl text-4xl font-black tracking-[-0.04em] text-white md:text-5xl">
                {t("download.pipeline.title", {
                  defaultValue: "A homepage section that now reflects the backend pipeline too",
                })}
              </h2>
            </div>
      ...
      <section className="px-4 pb-18 pt-8 md:px-8 md:pb-24 md:pt-10">
        <div className="mx-auto max-w-7xl rounded-[36px] border border-white/10 bg-white/6 px-6 py-10 backdrop-blur md:px-10 md:py-12">
          <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="mb-4 text-sm font-semibold uppercase tracking-[0.28em] text-cyan-200/75">
                {t("download.platforms.eyebrow", { defaultValue: "Ready to install" })}
              </p>
              <h2 className="text-4xl font-black tracking-[-0.04em] text-white md:text-5xl">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/6 p-3">
                          {platform.icon ? (
                            ...
                            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/8 text-xl">
                              {t("download.platforms.mobileEmoji", { defaultValue: "📱" })}
                            </div>
                          )}
                    </div>
                    <div>
                      <h3 className="text-xl font-bold text-white sm:text-2xl">{platform.name}</h3>
                      <p className="text-sm text-slate-400">Beta 2.0.0</p>
                    </div>
                  </div>
                </div>

                <p className="mt-5 min-h-18 text-sm leading-6 text-slate-300">{platform.description}</p>
